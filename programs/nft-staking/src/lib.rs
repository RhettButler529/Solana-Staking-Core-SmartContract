...
pub fn update_rewards(
    ...
    for i in 0..user_store.types.len() {
        ...
        user_store.staked_times[i] = current_time.checked_sub(diff_times).unwrap();
...
pub mod nft_staking {
    ...
    pub fn unstake(ctx: Context<Stake>) -> Result<()> {
        ...
        let index = user_store.nft_mints.iter().position(|x| *x == stake_to_account_mint);
        if index == None {
            return Err(ErrorCode::NotStakedNft.into());
        }
